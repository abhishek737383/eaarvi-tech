  ...
  return (
    <div className="rounded-lg border border-zinc-100 dark:border-zinc-800 overflow-hidden">
      <button
        onClick={() => setOpen((s) => !s)}
        aria-expanded={open}
        className="w-full flex items-center justify-between px-4 py-3 bg-white dark:bg-zinc-900/60 text-left text-sm font-medium focus:outline-none focus-visible:ring-2 focus-visible:ring-indigo-300"
      >
        <span>{title}</span>
        <motion.span
      ...
      <AnimatePresence initial={false}>
        {open && (
          <motion.div
            ...
            className="px-4 pt-3 pb-4 bg-white/60 dark:bg-zinc-900/50 text-sm text-zinc-600 dark:text-zinc-300"
          >
            {children}
          </motion.div>
        )}
...
function ServiceCard({ title, Icon, children, accent = "from-indigo-50 to-indigo-100" }) {
  ...
  return (
    <motion.article
      ...
      className="rounded-2xl p-5 md:p-6 bg-white/70 dark:bg-zinc-900/50 border border-zinc-100 dark:border-zinc-800 shadow-sm"
    >
    ...
    <main className="min-h-screen py-12 bg-gradient-to-b from-white to-zinc-50 dark:from-zinc-900 dark:to-zinc-800">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <motion.section
          ...
          className="rounded-3xl bg-white/70 dark:bg-zinc-900/50 p-6 sm:p-8 md:p-10 shadow-lg"
        >
          <div className="md:flex md:items-center md:justify-between gap-6">
            <motion.div variants={itemVariant} className="max-w-2xl">
              ...
              <div className="mt-5 flex flex-wrap gap-3">
                ...
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="inline-flex items-center gap-2 rounded-full border border-zinc-200 px-3 py-2 font-medium hover:bg-zinc-100 transition-colors"
                >
            ...
            <motion.div variants={itemVariant} className="mt-6 md:mt-0 md:w-96">
              <div className="rounded-xl overflow-hidden border border-zinc-100 dark:border-zinc-800 shadow-sm bg-gradient-to-br from-indigo-600 to-purple-500 text-white p-5">
        ...
        <motion.section initial="hidden" whileInView="show" viewport={{ once: true }} variants={containerVariants} className="mt-8 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          ...
          <motion.div variants={itemVariant} className="rounded-2xl p-5 bg-white/70 dark:bg-zinc-900/50 border border-zinc-100 dark:border-zinc-800 shadow-sm flex items-center justify-center">
            <div className="text-center">
              <div className="text-lg font-semibold">Custom Programs</div>
              <p className="mt-2 text-sm text-zinc-600 dark:text-zinc-300">Tailored content, assessments & on-site support — request a custom proposal for your industry.</p>
        ...
        <motion.section initial="hidden" whileInView="show" viewport={{ once: true }} variants={containerVariants} className="mt-8 grid grid-cols-1 lg:grid-cols-2 gap-6">
          <motion.div variants={itemVariant} className="rounded-2xl p-6 bg-white/60 dark:bg-zinc-900/50 shadow">
            <h3 className="text-xl font-semibold">Corporate Sales Training — Modules</h3>
          ...
          <motion.div variants={itemVariant} className="rounded-2xl p-6 bg-white/60 dark:bg-zinc-900/50 shadow">
            <h3 className="text-xl font-semibold">Software Training for Engineering Professionals</h3>
            <p className="mt-3 text-sm text-zinc-600 dark:text-zinc-300">Corporate and institutional training across key design & engineering tools — for working professionals and students.</p>
            ...
            <div className="mt-4 grid grid-cols-1 sm:grid-cols-2 gap-3">
              {[
                ...
                <div key={s.name} className="rounded-lg p-3 border border-zinc-100 dark:border-zinc-800">
            ...
            <div className="mt-6 flex gap-3 flex-wrap">
              ...
              <a href="/brochure.pdf" className="inline-flex items-center gap-2 rounded-full border border-zinc-200 px-4 py-2 font-medium hover:bg-zinc-100" target="_blank" rel="noreferrer"><BookOpen className="h-4 w-4" /> Download Syllabus</a>
            </div>
          </motion.div>
        </motion.section>
        ...
        <div className="mt-10 text-center text-sm text-zinc-500">© {new Date().getFullYear()} eAarvi Tech — Built for performance</div>
      </div>
    </main>
  );
}
