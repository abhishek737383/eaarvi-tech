        ...
        <nav className="hidden md:flex items-center gap-6 text-sm">
          {NAV_LINKS.map((l) => (
            <Link
              key={l.href}
              href={l.href}
              className="relative px-2 py-1 rounded-md group outline-none focus-visible:ring-2 focus-visible:ring-primary-300"
            >
              <span className="inline-block transition-transform duration-200 group-hover:-translate-y-0.5">
                {l.label}
              </span>
              <span className="absolute left-0 -bottom-1 w-full h-0.5 origin-left scale-x-0 group-hover:scale-x-100 transition-transform duration-200 bg-gradient-to-r from-primary-500 to-primary-700 rounded" />
            </Link>
          ))}
        </nav>
